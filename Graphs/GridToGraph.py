    ...
    def add_edges(self):
        for i in range(self.vertices):
            for j in self.vectors:
                if (i+1) % self.rows == 0 and j == 1:   #Last cell of a some row cannot be connected to first cell of next row
                    continue
                if j == -1 and i % self.rows == 0:  #First cell of some row cannot be connected to the last cell of previous row
                    continue
